...
AFRAME.registerComponent('a-terrain', {
  ...
  */
  ...
  tick: function tick() {
    ...
    this.updateView(); // Sweep old tiles if any
  ...
  updateView: function updateView() {
    ...
    var data = this.data;
    var observer = (data.observer && data.observer.length) > 0 ? this.el.sceneEl.querySelector("#" + data.observer) : 0;
    ...
    if (observer) {
      ...
      var v1 = this.el.object3D.getWorldPosition();
      var v2 = observer.object3D.getWorldPosition(); // get distance between them...

      var d = v1.distanceTo(v2); // find relative vector of unit length pointing at the observer

      var m = new THREE.Matrix4();
      m.getInverse(this.el.object3D.matrixWorld);
      v2.transformDirection(m);
      var lat = Math.asin(v2.y);
      var lon = Math.atan2(v2.x, v2.z); // Exit now if no significant change
      ...
      this.previous_lon = lon; // go to degrees (for user convenience - this is a bit messy tidy up later)

      data.latitude = lat * 180.0 / Math.PI;
      data.longitude = lon * 180.0 / Math.PI; // find planetary coordinate space distance from sealevel of ellipsoid (or a sphere as is the case in this engine)

      data.elevation = d * data.world_radius / data.radius - data.world_radius; // What is a pleasant level of detail for a given distance from the planets center in planetary coordinates?
      ...
      if (params.lat) {
        data.latitude = parseFloat(params.lat);
        data.longitude = parseFloat(params.lon);
      }
      ...
      if (data.follow & 2) {
        ...
        var height = data.radius * data.elevation * data.stretch / data.world_radius + data.radius;
      ...
      _TileServer.default.instance().getGround(data.latitude, data.longitude, data.lod, data.url, function (groundValue) {
        ...
        var e = groundValue + data.elevation; // convert elevation above sea level to to model scale

        var height = data.radius * e * data.stretch / data.world_radius + data.radius;
        console.log("Dynamically moving planet to adjust for ground=" + groundValue + " height=" + height + " stretch=" + data.stretch + " elev=" + e);

        _this.el.object3D.position.set(0, -height, 0);
      });
    } // Copy user values to internal - TODO remove this by code cleanup later - naming inconsistencies


    data.lat = data.latitude;
    data.lon = data.longitude; // TODO these limits are imposed by the current data sources - and should not be true for all data sources
    ...
    var fovpad = data.fovpad;

    for (var i = -count - fovpad; i < count + 1 + fovpad; i++) {
      for (var j = -count - fovpad; j < count + 1 + fovpad; j++) {
  ...
  updateOrCreateTile: function updateOrCreateTile(data) {
    ...
    if (element) {
      ...
      this.el.appendChild(element); // WARN set lod and loaded directly on the element right now because getAttribute() appears to sometimes not be set synchronously
  ...
  ///
  sweepTiles: function sweepTiles() {
    var lod = this.data.lod; // bail immediately and do not sweep if any tiles at the current lod are not ready

    var keys = Object.keys(this.tiles);

    for (var i = 0; i < keys.length; i++) {
      var element = this.tiles[keys[i]];

      if (element.lod == lod && element.loaded != 1) {
        var properties = element.getAttribute('a-tile');
        return;
      }
    }
    ...
    for (var _i = 0; _i < keys.length; _i++) {
      var _element = this.tiles[keys[_i]];

      if (_element.lod != lod && _element.getAttribute("visible") != false) {
        _element.setAttribute("visible", false);
      }
    }
